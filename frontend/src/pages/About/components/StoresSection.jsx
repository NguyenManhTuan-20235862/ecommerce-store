    ...
    >
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-6 mb-12">
        ...
        <button
          ...
          className="bg-[#0f172a] text-white px-6 py-3 rounded-full text-[11px] font-bold uppercase tracking-widest hover:bg-black transition self-start sm:self-auto"
        >
          Xem trên bản đồ →
        </button>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="border border-black/10 rounded-[2rem] p-4 flex flex-col sm:flex-row gap-6 animate-pulse"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {stores.map((store) => {
            ...
            return (
              <div
                key={store._id}
                className="border border-black/10 rounded-[2rem] p-4 flex flex-col sm:flex-row gap-6 hover:border-black/30 transition"
              >
                {/* Store Image */}
                <div
                  className={`w-full sm:w-[200px] h-[200px] rounded-[1.5rem] relative overflow-hidden ${store.imgUrl ? "" : colors.imgColor}`}
                >
                  {store.imgUrl ? (
                    <img
                  ...
                  <div className="absolute top-3 left-3">
                    <span
                      className={`inline-block px-2 py-1 text-[8px] font-bold uppercase tracking-widest rounded-full ${colors.tagColor}`}
                    >
